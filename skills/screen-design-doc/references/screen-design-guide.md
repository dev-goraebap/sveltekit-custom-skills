...
`ia-sitemap.md` 상단에 반드시 메타 테이블을 포함한다. 플랫폼과 레이아웃 정보를 명시한다.

```markdown
| 항목 | 값 |
|------|-----|
| 사이트 | (사이트 슬러그 또는 "default") |
| 대상 | (예: 관리자, 일반 사용자, 파트너) |
| 플랫폼 | (웹 데스크톱 / 모바일 웹 / 반응형) |
| 레이아웃 | (사이드바 / 하단탭 / 탑바) |
```

### 플랫폼별 레이아웃 힌트

| 플랫폼 | 기본 레이아웃 | max-width | 비고 |
|--------|-------------|-----------|------|
| 웹 데스크톱 | 사이드바 | 1200px | 좌측 사이드바 + 우측 콘텐츠 |
| 모바일 웹 | 하단탭 | 480px | 하단 고정 탭 바 |
| 반응형 | 탑바 | 100% | 미디어쿼리로 분기 |
...
멀티 사이트일 때 각 사이트별 `ia-sitemap.md`에 해당 사이트의 화면만 포함한다.
SC ID에 사이트 슬러그 대문자가 접두어로 붙는다: `SC-[SITE]-[그룹]-[순번]`
...
flowchart TD
    ...
    g01 --> sc0101["SC-ADMIN-01-01\n로그인"]
    g02 --> sc0201["SC-ADMIN-02-01\n대시보드"]
    g02 --> sc0202["SC-ADMIN-02-02\n회원 관리"]
```
...
flowchart TD
    ...
    g01 --> sc0101["SC-USER-01-01\n온보딩"]
    g02 --> sc0201["SC-USER-02-01\n홈"]
    g02 --> sc0202["SC-USER-02-02\n마이페이지"]
```
...
SC-ADMIN-01-01  ← admin 사이트, 그룹 01, 첫 번째 화면
SC-USER-02-03   ← user 사이트, 그룹 02, 세 번째 화면
```

- SITE: 사이트 슬러그를 대문자로 (예: `admin` → `ADMIN`, `user` → `USER`)
- 그룹 번호: 2자리 zero-padding (01, 02, ... 10, 11)
- 순번: 그룹 내 2자리 zero-padding
- 모달/오버레이: `SC-01-01-M` 또는 `SC-ADMIN-01-01-M` (ID에 `-M` 접미어)
